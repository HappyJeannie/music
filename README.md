...
歌曲列表和上传歌曲与这个的 js 的 MVC 格式类似。

#### 2、 MVC 结构之前数据通讯

MVC 之前数据交互使用命名空间实现。在 `src/admin/js` 下新建 `app.js` 文件，内容为：
```
window.app = {}
```

然后每个 MVC 文件提供一个对外输出，如 `song-upload.js` 最后一行增加代码：
```
window.app.songUpload = controller;
```
这样在其他的 MVC 文件中同样能够调用当前 js 的控制器中的属性和方法。

#### 3、发布订阅模式

像上一步那样引入命名空间，在不同的 js 中进行调用会存在一个很大的问题，就是模块之间耦合度比较高，解决这个问题的方式是使用 发布/订阅 模式。
举例说明发布/订阅模式：
现有 模块 1 、2 、3 、4 ，其中模块 1 数据发生变化，需要通知模块 2 和 模块 4 ， 现有方法是通过命名空间直接调用模块 2/4 的方法，发布订阅模式则是此时有公共模块 5 ， 当模块 1 发生数据变化，直接通知模块 5 ，模块 5 再将变化发送至模块 2/4。